...
export const projectsData: Projectdata[] = [
  ...
  },
  {
    ...
    details: `Since this was entirely a learning experience this game is not polished I'm afraid. My partner and I watch Jeopardy every day and we get annoyed that all the online versions of it are only available in America. So I made this app as a labour of love for the game and so that I could learn Angular, WebSocket and Angular Redux. It pulls info from a third party API endpoint and reworks the data into a MySQL database to store the games and scores etc.

     The most challenging part was changing my perspective to think about different clients connecting via WebSockets at the same time, as well as learning the different intricacies of Angular and how different it was from React. It does actually store individual names and scores for each game, and many people can play on the same game at once. It is very fussy with what answers it accepts. The original idea was to have the answers that the user enters be sent off to ChatGPT so it can check to see if they've given a repsonse that is correct but doesn't quite match the expected answer. This turned out to be too slow and and a lot of work. I'm proud of how fast I wrapped my head around Property Binding, Event Handlers and Observables. I had a particularly hard time getting the timers to work for when a contestant buzzes in, so that is a bit janky still.`,
    thingsUsed: [
      'Full Stack',
      'TypeScript',
      'WebSocket',
  ...
  },
  {
    ...
    details: `This game was my personal project during Dev Academy. I worked on it in my very small amount of spare time, which is why it's a bit messy with it's styling. It's a little similar to Wordle, in that the game will pick a random word from thedictionary. Your job is to guess the word by making letters out of matchsticks, kind of similar to in school how everyone used to try to make dirty words on the 7 segment displays of old calculators. If you don't guess the word correctly, the matchsticks that are in the right place will burn out giving you a hint on what letters are in the word. I've also implemented a Hint button which uses the ChatGPTAPI to help you guess the word. From this project I learned how frustrating CSS flex boxes can be, and the intricacies of using JSONfiles to store and return data. If I had to do this again, I would use Tailwind for CSS and I would be much more careful with how it looks on mobile. Right now it's a garbled mess on smaller screens. I also want to add the ability to do all letters in the alphabet, as right now you can't do letters like M and X. In order to achieve that I would make the matchsticks draggable to anywhere onscreen instead of being in a grid.`,
